...
void FastaIndex::readIndexFile(string fname) {
  ...
  if (indexFile.is_open())
  {
    while (getline (indexFile, line))
    {
      ...
      if (fields.size() == 5)
      {  // if we don't get enough fields then there is a problem with the file
      ...
      {
        std::cerr << "Warning: malformed fasta index file " << fname << 
            "does not have enough fields @ line " << linenum << std::endl;
  ...
  {
    std::cerr << "could not open index file " << fname << std::endl;
    exit(1);
  }
}

// for consistency this should be a class method
bool fastaIndexEntryCompare ( FastaIndexEntry a, FastaIndexEntry b)
{
...
ostream& operator<<(ostream& output, FastaIndex& fastaIndex)
{
  ...
  for (auto it = fastaIndex.sequenceNames.begin(); it != fastaIndex.sequenceNames.end(); ++it)
  {
  ...
  for (auto fit = sortedIndex.begin(); fit != sortedIndex.end(); ++fit)
  {
    output << *fit << endl;
  }
  return output;
...
void FastaIndex::indexReference(string refname)
{
  ...
  if (refFile.is_open())
  {
    while (getline(refFile, line))
    {
      ...
      { // we assume we have found a sequence line
        ...
        if (entry.line_len)
        {
          ...
          if (mismatchedLineLengths || emptyLine)
          {
            ...
            else
            {
              if (emptyLine)
              {
                std::cerr << "ERROR: embedded newline";
              }
              else
              {
                std::cerr << "ERROR: mismatched line lengths";
              }

              std::cerr << " at line " << line_number << " within sequence " << entry.name <<
              std::endl << "File not suitable for fasta index generation." << std::endl;
              exit(1);
            }
          }
  ...
  {
    std::cerr << "could not open reference file " << refname << " for indexing!" << std::endl;
    exit(1);
  }
}
...
string FastaReference::getSequence(std::string seqname)
{
...
seqan::String<seqan::Dna5> FastaReference::get_seqan_sequence(std::string seqname)
{
  FastaIndexEntry entry = index->entry(seqname);
  int newlines_in_sequence = entry.length / entry.line_blen;
  int seqlen = newlines_in_sequence  + entry.length;
  ...
  if (fread(seq, sizeof(char), seqlen, file))
  {
    ...
    s = seq;
    free(seq);
    resize(s, (pend - pbegin)/sizeof(char));
  }
  return s;
}
...
seqan::String<seqan::Dna5> FastaReference::get_seqan_target_region(FastaRegion & target)
{
  if (target.startPos == -1)
  {
    return get_seqan_sequence(target.startSeq);
  }
  else
  {
    return get_seqan_sub_sequence(target.startSeq, target.startPos - 1, target.length());
  }
}

seqan::String<seqan::Dna5> FastaReference::get_seqan_sub_sequence(string seqname, int start, int length)
{
  FastaIndexEntry entry = index->entry(seqname);
